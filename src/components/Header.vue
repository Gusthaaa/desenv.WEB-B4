<script>

</script>


<template>
    <header>
        <nav class="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
            <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                <div class="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
                    <ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0 justify-center align-middle justify-items-center">
                        <li>
                            <router-link 
                                to="/" 
                                class="flex py-2 pr-4 pl-3 text-gray-800 rounded lg:bg-transparent lg:text-blue-600 lg:p-0 dark:text-gray-200 dark:hover:text-white hover:text-blue-600" 
                                aria-current="page"
                            >
                                Home
                            </router-link>
                        </li>
                        <li>
                            <router-link 
                                to="/filmes" 
                                class="block py-2 pr-4 pl-3 text-gray-800 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-blue-600 lg:p-0 dark:text-gray-200 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700"
                            >
                                Catálogo de Filmes
                            </router-link>
                        </li>   
                        <li>
                            <router-link 
                                to="/cadastro" 
                                class="block py-2 pr-4 pl-3 text-gray-800 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-blue-600 lg:p-0 dark:text-gray-200 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700"
                            >
                                Cadastrar filmes
                            </router-link>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    </header>
</template>

